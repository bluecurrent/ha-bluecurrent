...
Configure a user override.

> **Note:** A charge point can only have one override schedule at a time.
> If a charge point already has an active override schedule and the Set User Override Current action is used again, the existing schedule will be overwritten.

| Data attribute        | Optional | Description                                                                                                                  |
| --------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `device_ids`          | no       | The ID's of the Blue Current charge point.                                                                                   |
| `current`             | no       | If the current that Smart Charging would advise is higher thant the user override, it will advise the user override instead. |
| `override_start_days` | no       | The days the override will start.                                                                                            |
| `override_start_time` | no       | The time the override will start.                                                                                            |
| `override_end_days`   | no       | The days the override will end.                                                                                              |
| `override_end_time`   | no       | The time the override will end.                                                                                              |

### Action clear_user_override

Clear the user override on a charge point.
